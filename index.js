// ============================================================
//  COTIZA.CHAT v6 — Bot WhatsApp Business
//  Sistema Nacional de Proveedores Ecuador 🇪🇨
//  ServicioNacionaldeCotizaciones.com
//  Flujo mejorado con formulario completo + FAQ + LOPDP
// ============================================================
...
// ── FAQ — palabras clave y respuestas ──────────────────────
const FAQ = [
  {
  ...
  },
  {
    ...
    respuesta:
      ...
      `   👉 RegistroNacionaldeProveedores.com\n\n` +
      `Consulta el perfil completo de cualquier\n` +
      `proveedor antes de cerrar tu compra. 🇪🇨`
  },
  {
    keywords: ["tiempo","cuánto tarda","cuando","demora","rápido","rapido"],
    respuesta:
      `⏱️ *¿Cuánto tarda en llegar mi cotización?*\n\n` +
      `📱 Los proveedores te contactan directamente\n` +
      `por WhatsApp en:\n\n` +
      `⚡ *30 minutos* — casos urgentes\n` +
      `📅 *2 a 4 horas hábiles* — tiempo normal\n` +
      `🌙 *Siguiente día hábil* — fuera de horario\n\n` +
      `_Horario de atención proveedores:_\n` +
      `Lun-Vie 8:00 - 18:00 · Sáb 9:00 - 13:00`
  },
  {
    keywords: ["humano","persona","hablar","asesor","ayuda","help","soporte"],
    respuesta:
      ...
      `con tu solicitud de cotización._ 🇪🇨`
  },
  {
    keywords: ["revista","directorio","catalogo","catálogo"],
    respuesta:
      `📰 *Revista de Proveedores Ecuador*\n\n` +
      `Encuentra el directorio completo de\n` +
      `proveedores verificados en:\n\n` +
      `👉 *RevistadeProveedores.com*\n\n` +
      `Filtra por categoría, ciudad y más. 🇪🇨`
  },
];
...
// ── Detectar emoji producto ────────────────────────────────
function emojiProducto(producto) {
  const p = producto.toLowerCase();
  if (/cemento|hierro|ladrillo|construc|material|ferret/.test(p)) return "🏗️";
  if (/computadora|laptop|tecnolog|equipo|hardware/.test(p))       return "💻";
  if (/uniforme|ropa|textil|camisa|tela/.test(p))                  return "👕";
  if (/limpieza|aseo|detergente|jabón/.test(p))                    return "🧹";
  if (/transport|flete|camión|logística/.test(p))                  return "🚛";
  if (/alimento|comida|bebida|víveres/.test(p))                    return "🍱";
  if (/maquina|motor|industrial|equipo/.test(p))                   return "⚙️";
  if (/médico|salud|farmacia|medicina/.test(p))                    return "🏥";
  if (/seguridad|guardia|vigilancia/.test(p))                      return "🛡️";
  if (/papel|oficina|escritorio|tóner/.test(p))                    return "📋";
  return "📦";
}
...
async function flujo(tel, texto) {
  ...
  if (t === "hablar_humano")  return hablarHumano(tel);
  ...
  if (faqResp && ses.estado === E.INICIO) {
    ...
    return enviarTexto(tel, "¿Hay algo más en lo que pueda ayudarte?\nEscribe *COTIZAR* para solicitar una cotización. 😊");
  }

  switch (ses.estado) {

    // ── INICIO → pedir producto ──────────────────────────
    case E.INICIO:
      return bienvenida(tel);

    // ── PRODUCTO capturado → pedir detalle ───────────────
    case E.PRODUCTO:
      ...
      return await enviarTexto(tel,
        `${emojiProducto(ses.datos.producto)} *${ses.datos.producto}* — anotado.\n\n` +
        `📝 Para enviarte las mejores cotizaciones necesitamos algunos datos.\n\n` +
        `Cuéntanos con *más detalle* lo que necesitas:\n\n` +
        `Incluye si puedes:\n` +
    ...
    // ── DETALLE → pedir nombre ───────────────────────────
    case E.DETALLE:
      ses.datos.detalle = texto;
      ses.estado = E.NOMBRE;
      return enviarTexto(tel, `👤 *¿Cuál es tu nombre completo?*`);

    // ── NOMBRE → pedir ciudad ────────────────────────────
    case E.NOMBRE:
      if (texto.length < 3) return enviarTexto(tel, "Por favor escribe tu nombre completo.");
    ...
    // ── CIUDAD ───────────────────────────────────────────
    case E.CIUDAD:
      if (texto === "CIUDAD_OTRA") {
        ses.estado = E.CIUDAD_OTRA;
        return enviarTexto(tel, "📍 ¿En qué ciudad necesitas el producto o servicio?");
      }
    ...
    case E.EMPRESA_OPC:
      if (t === "si_empresa") {
        ses.estado = E.EMPRESA_DATOS;
        return enviarTexto(tel,
          `🏢 Por favor escribe el *nombre de tu empresa* y tu *RUC*:\n\n` +
          `_Ej: Constructora del Sur · 0190123456001_`
        );
      } else {
        ses.datos.empresa = "Persona natural";
        ses.datos.ruc = "";
        ses.estado = E.EMAIL;
        return enviarTexto(tel, `📧 ¿Cuál es tu *correo electrónico*?\n_Ej: nombre@correo.com_`);
      }

    // ── DATOS EMPRESA ────────────────────────────────────
    case E.EMPRESA_DATOS:
      ses.datos.empresa_raw = texto;
      // Separar nombre y RUC
      const partes = texto.split(/[·\-,]/);
      ses.datos.empresa = partes[0]?.trim() || texto;
      ses.datos.ruc = partes[1]?.trim() || "";
      ses.estado = E.EMAIL;
      return enviarTexto(tel, `📧 ¿Cuál es tu *correo electrónico*?\n_Ej: nombre@correo.com_`);
    ...
    case E.CANT_COTS:
      ...
      return elegirCobertura(tel);

    // ── COBERTURA GEOGRÁFICA ─────────────────────────────
    case E.COBERTURA:
      ses.datos.cobertura = texto.replace("COB_","");
...
async function bienvenida(tel) {
  ...
  return enviarBotones(tel, {
    ...
    body   :
      ...
      "en minutos — sin llamadas, sin esperas.\n\n" +
...
async function hablarHumano(tel) {
  ...
  return enviarTexto(tel,
    ...
    `en breve por este mismo WhatsApp.\n\n` +
...
async function elegirCiudad(tel) {
  return enviarLista(tel, {
    body     : "📍 ¿En qué *ciudad* necesitas el producto o servicio?",
    ...
    secciones: [
      ...
      {
        titulo: "Nacional / Internacional",
        filas : [
          { id: "CIUDAD_Nacional",       titulo: "🇪🇨 Todo Ecuador"   },
          { id: "CIUDAD_Internacional",  titulo: "🌎 Internacional"   },
          { id: "CIUDAD_OTRA",           titulo: "📍 Otra ciudad"     },
        ]
      }
    ]
  });
...
async function preguntaEmpresa(tel) {
  return enviarBotones(tel, {
    body   : `🏢 ¿Tu solicitud es para una *empresa o negocio*?`,
    footer : "Esto nos ayuda a enviarte cotizaciones más precisas",
    botones: [
      { id: "si_empresa",  titulo: "🏢 Sí, tengo empresa" },
      { id: "no_empresa",  titulo: "👤 No, es personal"   },
    ]
  });
}
...
async function elegirCantidad(tel) {
  return enviarBotones(tel, {
    body   : `📊 ¿Cuántas *cotizaciones* deseas recibir?`,
    footer : "Más cotizaciones = mejores precios para comparar",
    botones: [
      { id: "CANT_1",  titulo: "1️⃣ Una cotización"  },
      { id: "CANT_3",  titulo: "3️⃣ Tres cotizaciones" },
      { id: "CANT_5",  titulo: "5️⃣ Cinco o más"       },
    ]
  });
}

async function elegirCobertura(tel) {
  return enviarBotones(tel, {
    body   : `🌎 ¿De dónde quieres recibir las cotizaciones?`,
    botones: [
      { id: "COB_ciudad",         titulo: "📍 Solo mi ciudad"    },
      { id: "COB_ecuador",        titulo: "🇪🇨 Todo Ecuador"     },
      { id: "COB_internacional",  titulo: "🌎 Internacionales"   },
    ]
  });
}
...
async function pedirAutorizacion(tel) {
  return enviarBotones(tel, {
    ...
    body   :
      ...
      `Consulta nuestra política en:\n` +
      `ServicioNacionaldeCotizaciones.com/politica-privacidad.html`,
...
async function finalizar(tel, ses) {
  ...
  const coberturaTexto = {
    "ciudad"        : `Solo ${d.ciudad}`,
    "ecuador"       : "Todo Ecuador",
    "internacional" : "Internacional",
  }[d.cobertura] || d.cobertura;
  ...
  cotizaciones.push({ folio: num, ...d, cobertura: coberturaTexto, telefono: tel, creado: ts });
  ...
  await enviarBotones(tel, {
    ...
    body   :
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `📄 Folio: *${num}*\n` +
      `${emojiProducto(d.producto)} Producto: *${d.producto}*\n` +
      `📍 Ciudad: *${d.ciudad}*\n` +
      `🌎 Cobertura: *${coberturaTexto}*\n` +
      `📊 Cotizaciones: *${cantTexto}*\n` +
      `━━━━━━━━━━━━━━━━━━━━\n\n` +
      `Los Proveedores Verificados te contactarán\n` +
    ...
    footer : "Powered by ServicioNacionaldeCotizaciones.com 🇪🇨",
    botones: [{ id: "nueva_cot", titulo: "🔄 Nueva cotización" }]
  });
  ...
  if (ADMIN_WA) {
    ...
    await enviarTexto(ADMIN_WA,
      ...
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `${emojiProducto(d.producto)} *Producto:* ${d.producto}\n` +
      `📝 *Detalle:* ${d.detalle?.slice(0,100) || "No especificado"}\n` +
      `👤 *Nombre:* ${d.nombre}\n` +
      `📍 *Ciudad:* ${d.ciudad}\n` +
      `🌎 *Cobertura:* ${coberturaTexto}\n` +
  ...
  console.log(`✅ Cotización: ${num} | ${d.nombre} | ${d.producto} | ${d.ciudad}`);
}
  ...
  if (t.startsWith("lead_si_")) {
    ...
    if (ADMIN_WA) {
      await enviarTexto(ADMIN_WA,
        ...
        `Proveedor: +${tel}\n` +
        `Cliente: ${cot?.nombre} — ${cot?.empresa || "Personal"}`
      );
    }
    return;
  }
...
app.get("/admin", (req, res) => {
  ...
  const filas = cotizaciones.length === 0
    ? `<tr><td colspan="8" style="text-align:center;padding:30px;color:#888">Sin cotizaciones aún</td></tr>`
    : [...cotizaciones].reverse().map((c,i) => `
      <tr style="background:${i%2===0?"#fff":"#f9fafb"}">
        <td><b style="color:#16a34a">${c.folio}</b></td>
        <td>${emojiProducto(c.producto)} ${c.producto}</td>
        <td>${c.nombre||""}</td>
        <td>${c.ciudad}</td>
...
<title>CotizaBot Admin v6</title>
<style>
...
.card{background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);margin-bottom:16px;overflow-x:auto}
...
<div class="top">
  <h1>🤖 CotizaBot v6 — Panel Admin</h1>
...
<div class="card"><table>
  <thead><tr><th>Folio</th><th>Producto</th><th>Nombre</th><th>Ciudad</th><th>Empresa</th><th>Email</th><th>Cobertura</th><th>Fecha</th></tr></thead>
  <tbody>${filas}</tbody>
</table></div>
</body></html>`);
});
...
app.listen(PORT, () => {
  console.log(`\n🚀 CotizaBot v6 corriendo en puerto ${PORT}`);
